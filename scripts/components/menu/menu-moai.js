/**
 * @file menu-moai.js
 * @desc Moai 菜单控制：绑定按钮点击、刷新多语言文本、触发动作或信息对话框
 * @pc-attrs
...
var MenuMoai = pc.createScript('MenuMoai');

MenuMoai.attributes.add('buttonEntities', {
    type: 'entity',
    array: true,
    title: '按钮实体数组',
...
MenuMoai.attributes.add('buttonTextKeys', {
    type: 'string',
    array: true,
    title: '按钮文本多语言键',
...
MenuMoai.attributes.add('buttonActionEvents', {
    type: 'string',
    array: true,
    title: '按钮动作事件',
...
MenuMoai.attributes.add('buttonDialogKeys', {
    type: 'string',
    array: true,
    title: '按钮对话框键',
...
MenuMoai.attributes.add('panelTitleKey', {
    type: 'string',
    title: '面板标题键',
    description: '如 menu.panels.title.logo',
    default: ''
});

MenuMoai.attributes.add('enableDebugLog', {
    type: 'boolean',
    default: false,
    title: '调试日志'
});

MenuMoai.prototype.initialize = function () {
    this._boundClicks = [];
...
MenuMoai.prototype.destroy = function () {
...
MenuMoai.prototype._bindButtons = function () {
    var self = this;
    ...
    for (var i = 0; i < len; i++) {
        ...
        (function (index, entity) {
            ...
            if (entity.button) {
                ...
                console.warn('[MenuMoai] Button entity without Button/Element:', entity && entity.name);
            }
        })(i, btnEnt);
    }
};

MenuMoai.prototype._onButtonClick = function (index, entity) {
    if (this.enableDebugLog) {
        console.log('[MenuMoai] Click index=' + index + ' entity=' + (entity && entity.name));
    }
    ...
    if (actionEv) {
        this.app.fire(actionEv, { source: 'MenuMoai', index: index, entity: entity });
    }
};

MenuMoai.prototype.refreshTexts = function () {
...
MenuMoai.prototype._dialogConfigByKey = function (k) {
...
MenuMoai.prototype._t = function (key, fallback) {
...
MenuMoai.prototype._findTitleEntity = function () {
...
MenuMoai.prototype._setEntityText = function (entity, text) {
    if (!entity) return;
