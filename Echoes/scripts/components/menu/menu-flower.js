/**
 * @file menu-flower.js
 * @desc Flower 菜单控制：绑定按钮点击、刷新多语言文本、触发动作或信息对话框
 * @pc-attrs
...
var MenuFlower = pc.createScript('MenuFlower');

MenuFlower.attributes.add('buttonEntities', {
    type: 'entity',
    array: true,
    title: '按钮实体数组',
...
MenuFlower.attributes.add('buttonTextKeys', {
    type: 'string',
    array: true,
    title: '按钮文本多语言键',
...
MenuFlower.attributes.add('buttonActionEvents', {
    type: 'string',
    array: true,
    title: '按钮动作事件',
...
MenuFlower.attributes.add('buttonDialogKeys', {
    type: 'string',
    array: true,
    title: '按钮对话框键',
...
MenuFlower.attributes.add('panelTitleKey', {
    type: 'string',
    title: '面板标题键',
    description: '如 menu.panels.title.flower',
    default: ''
});

MenuFlower.attributes.add('enableDebugLog', {
    type: 'boolean',
    default: false,
    title: '调试日志'
});

MenuFlower.prototype.initialize = function () {
    this._boundClicks = [];
...
MenuFlower.prototype.destroy = function () {
...
MenuFlower.prototype._bindButtons = function () {
    var self = this;
    ...
    for (var i = 0; i < len; i++) {
        ...
        (function (index, entity) {
            ...
            if (entity.button) {
                ...
                console.warn('[MenuFlower] Button entity without Button/Element:', entity && entity.name);
            }
        })(i, btnEnt);
    }
};

MenuFlower.prototype._onButtonClick = function (index, entity) {
    if (this.enableDebugLog) {
        console.log('[MenuFlower] Click index=' + index + ' entity=' + (entity && entity.name));
    }
    ...
    if (actionEv) {
        this.app.fire(actionEv, { source: 'MenuFlower', index: index, entity: entity });
    }
};

MenuFlower.prototype.refreshTexts = function () {
...
MenuFlower.prototype._dialogConfigByKey = function (k) {
...
MenuFlower.prototype._t = function (key, fallback) {
...
MenuFlower.prototype._findTitleEntity = function () {
...
MenuFlower.prototype._setEntityText = function (entity, text) {
    if (!entity) return;
